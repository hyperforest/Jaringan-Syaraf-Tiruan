...
# preprocess data
X_train, y_train, X_test, y_test = train_test_split(X, y, random_state=random_state)
...
model = LVQ(n_classes=n_classes, input_shape=input_shape,
	distance='euclidean', random_state=random_state)

model.fit(X_train, y_train, epochs=epochs, lr=lr, val_data=(X_test, y_test))
